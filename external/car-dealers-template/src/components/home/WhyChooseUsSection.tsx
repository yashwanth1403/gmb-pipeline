import { BUSINESS_NAME, BUSINESS_CITY } from "@/config/business";
import Container from "@/components/Container";
import { ShieldCheck, FileCheck, Gauge, ClipboardList } from "lucide-react";

const FEATURES = [
  {
    icon: ShieldCheck,
    title: "Non-Accident Cars",
    desc: "Every car passes a thorough accident history check — no hidden damage, ever.",
    accent: "from-blue-400 to-blue-500",
    num: "01",
  },
  {
    icon: FileCheck,
    title: "3-Day RC Transfer",
    desc: "Ownership transferred in your name within 3 days. Fast, hassle-free paperwork.",
    accent: "from-blue-500 to-blue-600",
    num: "02",
  },
  {
    icon: Gauge,
    title: "Odometer Authenticity",
    desc: "We verify odometer readings independently. No rollbacks, no surprises.",
    accent: "from-logo-blue to-primary",
    num: "03",
  },
  {
    icon: ClipboardList,
    title: "Service History Check",
    desc: "Full service and maintenance records reviewed before any car hits our lot.",
    accent: "from-blue-700 to-blue-800",
    num: "04",
  },
];

const WhyChooseUsSection = () => {
  return (
    <section className="py-16 sm:py-24 bg-[#0f172a] relative overflow-hidden">
      {/* subtle background grid */}
      <div
        className="absolute inset-0 opacity-[0.04]"
        style={{
          backgroundImage:
            "linear-gradient(#fff 1px,transparent 1px),linear-gradient(90deg,#fff 1px,transparent 1px)",
          backgroundSize: "40px 40px",
        }}
        aria-hidden
      />

      <Container className="relative space-y-12">
        {/* heading */}
        <div className="text-center space-y-3">
          <p className="text-sm font-semibold tracking-widest text-logo-blue-light uppercase">
            Why choose us
          </p>
          <h2 className="text-3xl font-extrabold sm:text-4xl text-white">
            Why Choose {BUSINESS_NAME}?
          </h2>
          <p className="text-slate-400 max-w-md mx-auto">
            Trusted by {BUSINESS_CITY} car buyers who value honesty and transparency
          </p>
        </div>

        {/* cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className={`group relative rounded-2xl bg-white/5 border border-white/10 p-6 flex flex-col gap-4 hover:bg-white/10 hover:-translate-y-1 transition-all duration-300`}
            >
              {/* number */}
              <span className="absolute top-4 right-5 text-4xl font-black text-white/5 select-none leading-none">
                {f.num}
              </span>

              {/* icon */}
              <div
                className={`w-12 h-12 rounded-xl bg-gradient-to-br ${f.accent} flex items-center justify-center shadow-lg`}
              >
                <f.icon className="h-6 w-6 text-white" />
              </div>

              {/* text */}
              <div className="space-y-1.5">
                <h3 className="font-bold text-white text-base">{f.title}</h3>
                <p className="text-sm text-slate-400 leading-relaxed">{f.desc}</p>
              </div>
            </div>
          ))}
        </div>
      </Container>
    </section>
  );
};

export default WhyChooseUsSection;
